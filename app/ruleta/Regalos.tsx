            ...
            {enviado ?
                ...
                <div className="flex flex-col gap-6 w-full md:sm:w-[50%]">
                    ...
                    <ol className='list-decimal'>{lista?.map((present, i) => {
                        return (
                            <li key={present + i} className='flex justify-between gap-4 max-w-[50%]'>{present.includes("http") ? <a className='underline cursor-pointer text-[0.5rem] md:sm:text-[1rem] text-sky-500' href={present} target='_blank'>{present}</a> : <p>{present}</p>}<button className='text-red-500' onClick={() => eliminarRegalo(i)}>X</button></li>
                        )
                    })}</ol>
                    <button onClick={() => {
