                    ...
                    {nombre.aRegalar === "" ?                        
                        ...
                        <div className="flex flex-col items-center justify-center gap-6 w-screen">
                            ...
                            <ol className='list-decimal w-fit p-2'>{nombreAmigo?.regalos.map((present, i) => {
                                return (
                                    <li key={present + i} className='flex justify-between text-[1.5rem] md:sm:text-[2rem]'>{present.includes("http") ? <a className='underline cursor-pointer text-[0.5rem] md:sm:text-[1rem] text-sky-500 w-screen p-4' href={present} target='_blank'>{i+1}. {present}</a> : <p>{i+1}. {present}</p>}</li>
                                )
                            })}</ol>
                        </div>
                    }
